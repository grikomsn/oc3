// Upstream retry analysis, adapted from opencode-copilot-chat
// src/provider/retry.ts (same intent: remove rejected fields, patch context
// overflow, retry only transient failures).
...
function patchContextOverflow(message: string, body: Record<string, unknown>): RetryPatch | undefined {
  ...
  const current = key ? positive(body[key]) : undefined;
